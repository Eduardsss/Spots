  ...
  return (
    <div className="spotz-modal-overlay" role="dialog" aria-modal="true">
      <form className="spotz-modal" onSubmit={handleSubmit}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', color: palette.textPrimary }}>
...
export default function MapPage() {
  ...
  const handleMapClick = (event: google.maps.MapMouseEvent) => {
    ...
    setFormState({
      mode: 'create',
      position: { lat, lng },
      values: { name: '', description: '', status: 'public', tags: [], images: [] },
    });
  };
  ...
  const handleCreateSubmit = async (values: SpotFormSubmission) => {
    ...
    try {
      ...
      const createdSpot = {
        ...response.spot,
        images: Array.isArray(response.spot.images) ? response.spot.images : [],
      };

      setSpots((current) => [...current, createdSpot]);
      if (createdSpot.tags?.length) {
  ...
  const handleEditSubmit = async (values: SpotFormSubmission) => {
    ...
    if (!areImageListsEqual(values.images, target.images ?? [])) {
  ...
  const handleReportSpot = async (spot: Spot) => {
    if (!currentUser) {
      setNotification({ type: 'error', message: 'Sign in to report spots.' });
      return;
    }

    const reasonInput = typeof window !== 'undefined'
      ? window.prompt('Let us know what is wrong (optional):') ?? ''
      : '';
    const reason = reasonInput.trim();

    try {
      await apiFetch(`/spots/${spot.id}/report`, {
        method: 'POST',
        body: { reason },
      });
      setNotification({ type: 'success', message: 'Report submitted. Thank you.' });
    } catch (error) {
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
              {currentUser && currentUser.id !== selectedSpot.owner.id ? (
                <button
                  type="button"
    ...
    <div style={{ height: 'calc(100vh - 160px)', minHeight: '520px', position: 'relative' }}>
      ...
      <SpotFormModal
        ...
        initialValues={
          formState
            ? formState.values
            : { name: '', description: '', status: 'public', tags: [], images: [] }
        }
