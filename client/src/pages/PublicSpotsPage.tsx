...
function SpotCard({
  ...
  return (
    ...
    >
      <SpotImageCarousel images={spot.images ?? []} height={200} />
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {currentUser && currentUser.id !== spot.user_id ? (
              <button
                ...
                style={{ padding: '8px 16px', borderRadius: radii.md }}
              >
                Report
              </button>
            ) : null}
...
export default function PublicSpotsPage() {
  ...
  useEffect(() => {
    ...
    const loadSpots = async () => {
      ...
      try {
        ...
        const sanitized = (response.spots ?? [])
          .filter((spot) => spot.status === 'public')
          .map((spot) => ({
            ...spot,
  ...
  const handleReportSpot = async (spot: Spot) => {
    if (!user) {
      window.alert('Sign in to report spots.');
      return;
    }

    const reasonInput = window.prompt('Let us know what is wrong (optional):') ?? '';
